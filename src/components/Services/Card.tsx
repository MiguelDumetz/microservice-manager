...
function MicroserviceCard({ id: _id, name, url, isSelecting = false, isSelected = false, onToggleSelect, onEdit, onDelete, showGraph = false }: MicroserviceCardProps) {
  ...
  const { data = { status: 'dead' as ServiceStatus } } = useQuery<QueryResult>({
    ...
    queryFn: async (): Promise<QueryResult> => {
      ...
      try {
        ...
        latencyHistoryRef.current = [...latencyHistoryRef.current.slice(-29), latency];
  ...
  const showChart = showGraph && (status === 'dead' || status === 'error' || latencyHistoryRef.current.length >= 2);
    ...
    <>
      <div
        ...
        className={[
          ...
          isSelected ? 'ring-2 ring-red-500/60 border-red-300 dark:border-red-500/30' : styles.border,
        ].join(' ')}
      >
        <div className="flex items-center gap-2 pr-9 lg:pr-11 2xl:pr-14">
          <span className={`w-2 h-2 lg:w-2.5 lg:h-2.5 2xl:w-3 2xl:h-3 rounded-full shrink-0 ${styles.dot}`} />
          <h2 className="text-sm lg:text-base 2xl:text-lg font-semibold text-gray-900 dark:text-white">{name}</h2>
        </div>
        <p className="text-xs 2xl:text-sm font-mono text-gray-500 dark:text-slate-400">{url}</p>
        <div className="flex items-center gap-2">
          <span className={`text-xs 2xl:text-sm font-semibold uppercase tracking-wider ${styles.label}`}>
            {status}{errorCode ? ` ${errorCode}` : ''}
          </span>
          {status === 'running' && latency !== undefined && (
            <span className="text-xs font-mono text-gray-400 dark:text-slate-500">{latency}ms</span>
          )}
        </div>
        {showChart && (
          <>
            <div className="-mx-3 lg:-mx-5 2xl:-mx-7 border-t border-gray-100 dark:border-slate-700/60" />
            <LatencyChart history={latencyHistoryRef.current} status={status} />
          </>
        )}
